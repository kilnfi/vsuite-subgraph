import {
  AppliedFixes
} from "../generated/templates/Cub/Cub"
import { Cub, Fix } from "../generated/schema"

export function handleAppliedFixed(event: AppliedFixes): void {
  const cub = Cub.load(event.address)
  
  for (let idx = 0; idx < event.params.fixes.length; ++idx) {
    const fixId = "fix@" + event.address.toHexString() + "@" + event.params.fixes[idx].toHexString() + "@" + event.transaction.hash.toHexString() + "@" + event.transactionLogIndex.toString()
