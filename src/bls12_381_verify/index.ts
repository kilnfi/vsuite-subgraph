// manual transpilation of https://github.com/paulmillr/noble-bls12-381
import { log } from 'matchstick-as';
import { BigInt as BI, Bytes } from '@graphprotocol/graph-ts';
...
class PointG1orError {
  value: PointG1 | null;
  error: string | null;
}

class FPArrayorError {
  value: Array<Fp> | null;
  error: string | null;
}

class FP2ArrayorError {
  value: Array<Fp2> | null;
  error: string | null;
}

class PointG2orError {
  value: PointG2 | null;
  error: string | null;
}
...
class FP2_2DArrayorError {
  value: Array<Array<Fp2>> | null;
  error: string | null;
}
...
class BI_2DArrayorError {
  value: Array<Array<BI>> | null;
  error: string | null;
}
...
class Fp {
  ...
  pow(n: BI): FporError {
    const pm_res = powMod(this.value, n, Fp.ORDER);
    if (pm_res.error != null) {
      return {
        value: null,
        error: pm_res.error
      };
  ...
  sqrt(): FporError | null {
    const root_res = this.pow(Fp.ORDER.plus(BI.fromI32(1)).div(BI.fromI32(4)));
    if (root_res.error != null) {
      return {
        value: null,
        error: root_res.error
      };
    }
    const root = root_res.value!;
  ...
  invert(): FporError {
    const res = invert(this.value, Fp.ORDER);
    if (res.error != null) {
      return {
        value: null,
        error: res.error
...
class Fp2 {
  ...
  sqrt(): Fp2orError | null {
    ...
    const check_res = candidateSqrt.square().div(this);
    if (check_res.error != null) {
      return {
        value: null,
        error: check_res.error
      };
    }
    const check = check_res.value!;
    ...
    const x1_res = candidateSqrt.div(root);
    if (x1_res.error != null) {
      return {
        value: null,
        error: x1_res.error
      };
    }
    const x1 = x1_res.value!;
    const x2 = x1.negate();
    const reim1 = x1.reim();
    const re1 = reim1.re;
    const im1 = reim1.im;
    ...
    const re2 = reim2.re;
    const im2 = reim2.im;
    if (im1.gt(im2) || (im1.equals(im2) && re1.gt(re2)))
      return {
        value: x1,
        error: null
      };
  ...
  invert(): Fp2orError {
    ...
    const a = reim.re;
    const b = reim.im;
    const res = new Fp(a.times(a).plus(b.times(b))).invert();
    if (res.error != null) {
      return {
        value: null,
        error: res.error
    ...
    const factor = res.value!;
    return {
      value: new Fp2(factor.multiply(new Fp(a)), factor.multiply(new Fp(b.times(BI_neg_one)))),
      error: null
    };
  }

  div(rhs: Fp2): Fp2orError {
    const res = rhs.invert();
    if (res.error != null) {
      return {
        value: null,
        error: res.error
    ...
    const inv = res.value!;
    return {
      value: this.multiply(inv),
      error: null
    };
  }

  divBI(rhs: BI): Fp2orError {
    const res = new Fp(rhs).invert();
    if (res.error != null) {
      return {
        value: null,
        error: res.error
    ...
    const inv = res.value!;
    return {
      value: this.multiplyBI(inv.value),
      error: null
    };
  }
...
class Fp6 {
  ...
  invert(): Fp6orError {
    ...
    const t4_res = c2.multiply(t1).add(c1.multiply(t2)).mulByNonresidue().add(c0.multiply(t0)).invert();
    if (t4_res.error != null) {
      return {
        value: null,
        error: t4_res.error
      };
    }
    const t4 = t4_res.value!;

    return {
      value: new Fp6(t4.multiply(t0), t4.multiply(t1), t4.multiply(t2)),
      error: null
    };
  }
...
class Fp12 {
  ...
  finalExponentiate(): Fp12orError {
    const x = CURVE.x;
    // this^(q⁶) / this
    const t0_res = this.frobeniusMap(6).div(this);
    if (t0_res.error != null) {
      return {
        value: null,
        error: t0_res.error
      };
    }
    const t0 = t0_res.value!;
    // t0^(q²) * t0
    ...
    return {
      value: t2_t5_pow_q2.multiply(t4_t1_pow_q3).multiply(t6_t1c_pow_q1).multiply(t7_t3c_t1),
      error: null
    };
  }

  div(rhs: Fp12): Fp12orError {
    const res = rhs.invert();
    if (res.error != null) {
      return {
        value: null,
  ...
  divBI(rhs: BI): Fp12orError {
    ...
    const inv = res.value!.value;
    return {
      value: this.multiplyBI(inv),
      error: null
    };
  }
  ...
  invert(): Fp12orError {
    const c0 = this.c0;
    const c1 = this.c1;
    const t_res = c0.square().subtract(c1.square().mulByNonresidue()).invert(); // 1 / (c0² - c1² * v)
    if (t_res.error != null) {
      return {
        value: null,
        error: t_res.error
    ...
    const t = t_res.value!;
    return {
      value: new Fp12(c0.multiply(t), c1.multiply(t).negate()), // (C0 * T) + (-C1 * T) * w
      error: null
    };
  }
...
class PointG1 {
  ...
  static fromHex(bytes: Uint8Array): PointG1orError {
    let point: PointG1;
    if (bytes.length == 48) {
      const P = CURVE.P;
      ...
      const x_pow_res = x.pow(BI.fromI32(3));
      if (x_pow_res.error != null) {
        return {
          value: null,
          error: x_pow_res.error
        };
      }
      const x_pow = x_pow_res.value!;
      const right = x_pow.plus(new Fp(CURVE.b));
      let y_res = right.sqrt();
      if (y_res == null) {
        return {
          value: null,
  ...
  assertValidity(): PointG1orError {
    ...
    const isOnCurve = this.isOnCurve();
    if (isOnCurve.error != null) {
      return {
        value: null,
    ...
    const isTorsionFree = this.isTorsionFree();
    if (isTorsionFree.error != null) {
      return {
        value: null,
        error: isTorsionFree.error
      };
    }
  ...
  private validateScalar(n: BI): BIorError {
    if (n.le(BI.zero()) || n.gt(CURVE.r)) {
      return {
        value: null,
        error: `Point#multiply: invalid scalar, expected positive integer < CURVE.r. Got: ${n}`
      };
    }
    return {
      value: n,
  ...
  multiplyUnsafe(scalar: BI): PointG1orError {
    const n_res = this.validateScalar(scalar);
    if (n_res.error != null) {
      return {
        value: null,
        error: n_res.error
  ...
  private mulCurveMinusX(): PointG1orError {
    return this.multiplyUnsafe(CURVE.x);
  }

  private mulCurveX(): PointG1orError {
    const mulu_res = this.multiplyUnsafe(CURVE.x);
    if (mulu_res.error != null) {
      return {
        value: null,
        error: mulu_res.error
      };
    }
    const mulu = mulu_res.value!;
    return {
      value: mulu.negate(),
      error: null
    };
  ...
  private isTorsionFree(): BooleanOrError {
    // todo: unroll
    const xP_res = this.mulCurveX(); // [x]P
    if (xP_res.error != null) {
      return {
        value: false,
        error: xP_res.error
      };
    }
    const xP = xP_res.value!;
    const u2P_res = xP.mulCurveMinusX(); // [u2]P
    if (u2P_res.error != null) {
      return {
        value: false,
        error: u2P_res.error
      };
    }
    const u2P = u2P_res.value!;
    return {
      value: u2P.equals(this.phi()),
      error: null
    };
  ...
  private isOnCurve(): BooleanOrError {
    const b = new Fp(CURVE.b);
    const x = this.x;
    const y = this.y;
    const z = this.z;
    const y_pow_res = y.pow(BI.fromI32(2));
    if (y_pow_res.error != null) {
      return {
        value: false,
        error: y_pow_res.error
      };
    }
    const y_pow = y_pow_res.value!;
    const x_pow_res = x.pow(BI.fromI32(3));
    if (x_pow_res.error != null) {
      return {
        value: false,
    ...
    const x_pow = x_pow_res.value!;
    const left = y_pow.multiply(z).subtract(x_pow);
    const z_pow_res = z.pow(BI.fromI32(3));
    if (z_pow_res.error != null) {
      return {
        value: false,
        error: z_pow_res.error
      };
    }
    const z_pow = z_pow_res.value!;
    const right = b.multiply(z_pow);
    return {
      value: left.subtract(right).isZero(),
      error: null
    };
  }

  millerLoop(P: PointG2): Fp12orError {
    ...
    const pp_res = P.pairingPrecomputes();
    if (pp_res.error != null) {
      return {
        value: null,
        error: pp_res.error
      };
    }
    const pp = pp_res.value!;
    return {
      value: millerLoop(pp, res.value!),
      error: null
    };
  }

  toAffine(invZ: Fp | null = null): FPArrayorError {
    const x = this.x;
    const y = this.y;
    const z = this.z;
...
class PointG2 {
  ...
  static fromSignature(hex: Uint8Array): PointG2orError {
    const P = CURVE.P;
    const half = hex.length / 2;
    if (half !== 48 && half !== 96) {
      return {
        value: null,
        error: 'Invalid compressed signature length, must be 96 or 192'
      };
    }
    ...
    if (y_res == null) {
      return {
        value: null,
        error: 'Failed to find a square root'
      };
    }
    if (y_res.error != null) {
      return {
    ...
    const res = point.assertValidity();
    if (res.error != null) {
      return {
        value: null,
        error: res.error
  ...
  static hashToCurve(msg: Uint8Array): PointG2orError {
    const u_res = hash_to_field(msg, 2);
    if (u_res.error != null) {
      return {
        value: null,
        error: u_res.error
    ...
    const u = u_res.value!;
    const x0xy_res = map_to_curve_simple_swu_9mod16(Fp2.fromBigTuple(u[0]));
    if (x0xy_res.error != null) {
      return {
        value: null,
        error: x0xy_res.error
      };
    }
    const x0xy = x0xy_res.value!;
    const x0 = x0xy[0];
    const y0 = x0xy[1];
    const x1y1_res = map_to_curve_simple_swu_9mod16(Fp2.fromBigTuple(u[1]));
    if (x1y1_res.error != null) {
      return {
        value: null,
        error: x1y1_res.error
      };
    }
    const x1y1 = x1y1_res.value!;
    const x1 = x1y1[0];
    const y1 = x1y1[1];
    const x2y2_res = new PointG2(x0, y0).plus(new PointG2(x1, y1)).toAffine(null);
    if (x2y2_res.error != null) {
      return {
        value: null,
        error: x2y2_res.error
      };
    }
    const x2y2 = x2y2_res.value!;
    const x2 = x2y2[0];
    const y2 = x2y2[1];
    const x3y3_res = isogenyMapG2(x2, y2);
    if (x3y3_res.error != null) {
      return {
        value: null,
        error: x3y3_res.error
      };
    }
    const x3y3 = x3y3_res.value!;
    const x3 = x3y3[0];
    const y3 = x3y3[1];
    return new PointG2(x3, y3).clearCofactor();
  }

  private psi(): PointG2orError {
    ...
    return {
      value: this.fromAffineTuple(psi(res.value![0], res.value![1])),
      error: null
    };
  }

  private psi2(): PointG2orError {
    const res = this.toAffine(null);
    if (res.error != null) {
      return {
        value: null,
    ...
    return {
      value: this.fromAffineTuple(psi2(res.value![0], res.value![1])),
      error: null
    };
  }
  ...
  multiplyUnsafe(scalar: BI): PointG2orError {
    const n_res = this.validateScalar(scalar);
    if (n_res.error != null) {
      return {
        value: null,
  ...
  private mulCurveX(): PointG2orError {
    const mulu_res = this.multiplyUnsafe(CURVE.x);
  ...
  clearCofactor(): PointG2orError {
    const P = this;
    const t1_res = P.mulCurveX(); // [-x]P
    if (t1_res.error != null) {
      return {
        value: null,
        error: t1_res.error
      };
    }
    const t1 = t1_res.value!;
    const t2_res = P.psi(); // Ψ(P)
    if (t2_res.error != null) {
      return {
        value: null,
        error: t2_res.error
      };
    ...
    let t2 = t2_res.value!;
    let t3 = P.double(); // 2P
    const t3_psi2_res = t3.psi2(); // Ψ²(2P)
    if (t3_psi2_res.error != null) {
      return {
        value: null,
        error: t3_psi2_res.error
      };
    }
    t3 = t3_psi2_res.value!;
    t3 = t3.subtract(t2); // Ψ²(2P) - Ψ(P)
    t2 = t1.plus(t2); // [-x]P + Ψ(P)
    const t2_mcx_res = t2.mulCurveX(); // [x²]P - [x]Ψ(P)
    if (t2_mcx_res.error != null) {
      return {
        value: null,
        error: t2_mcx_res.error
      };
    }
    t2 = t2_mcx_res.value!;
  ...
  toAffine(invZ: Fp2 | null): FP2ArrayorError {
    const x = this.x;
    const y = this.y;
    const z = this.z;
    const is0 = this.isZero();
  ...
  private isTorsionFree(): BooleanOrError {
    const P = this;
    const P_psi_res = P.psi();
    if (P_psi_res.error != null) {
      return {
        value: false,
        error: P_psi_res.error
      };
    }
    const P_mcx_res = P.mulCurveX();
    if (P_mcx_res.error != null) {
      return {
        value: false,
        error: P_mcx_res.error
      };
    }
    return {
      value: P_mcx_res.value!.equals(P_psi_res.value!), // ψ(P) == [u](P)
      error: null
    };
    // https://eprint.iacr.org/2019/814.pdf
  ...
  assertValidity(): PointG2orError {
    ...
    if (!this.isOnCurve()) {
      return {
        value: null,
        error: 'Invalid G2 point: not on curve Fp2'
      };
    }
    const torsionFree_res = this.isTorsionFree();
    if (torsionFree_res.error != null) {
      return {
        value: null,
  ...
  pairingPrecomputes(): FP2_2DArrayorError {
    if (this._PPRECOMPUTES != null) {
      return {
        value: this._PPRECOMPUTES as Array<Array<Fp2>>,
        error: null
      };
    }
    const affines_res = this.toAffine(null);
    if (affines_res.error != null) {
      return {
        value: null,
        error: affines_res.error
      };
    }
    const affines = affines_res.value!;
    const precomputes_res = calcPairingPrecomputes(affines[0], affines[1]);
    if (precomputes_res.error != null) {
      return {
        value: null,
        error: precomputes_res.error
      };
    }
    this._PPRECOMPUTES = precomputes_res.value!;
    return {
      value: this._PPRECOMPUTES as Array<Array<Fp2>>,
      error: null
    };
...
const wsq_inv = wsq_inv_wcu_inv.value![0];
const wcu_inv = wsq_inv_wcu_inv.value![1];
...
export function biFromHex(hex: string): BI {
...
function invert(num: BI, modulo: BI = CURVE.P): BIorError {
  const _0n = BI.zero();
  const _1n = BI_one;
  if (num.equals(_0n) || modulo.le(_0n)) {
    return {
      value: null,
      error: `invert: expected positive integers, got n=${num} mod=${modulo}`
    };
  }
...
function powMod(num: BI, power: BI, modulo: BI): BIorError {
  if (modulo.le(BI.zero()) || power.lt(BI.zero())) {
    return {
      value: null,
      error: `powMod: expected positive integers, got n=${num} power=${power} mod=${modulo}`
    };
  }
  if (modulo.equals(BI_one)) {
    return {
      value: BI.zero(),
      error: null
    };
...
function i2osp(value: i32, length: i32): U8AorError {
  if (value < 0 || value >= 1 << (8 * length)) {
    return {
      value: null,
      error: `bad I2OSP call: value=${value} length=${length}`
    };
  }
...
function expand_message_xmd(msg: Uint8Array, DST: Uint8Array, lenInBytes: f64, H: Hash = sha256): U8AorError {
  ...
  if (ell > 255) {
    return {
      value: null,
      error: 'Invalid xmd length'
    };
  }
  const i2osp_res_1 = i2osp(DST.length, 1);
  if (i2osp_res_1.error != null) {
    return {
      value: null,
      error: i2osp_res_1.error
    };
  }
  const i2osp_1 = i2osp_res_1.value!;
  const DST_prime = concatBytes([DST, i2osp_1]);
  const Z_pad_res = i2osp(0, i32(r_in_bytes));
  if (Z_pad_res.error != null) {
    return {
      value: null,
      error: Z_pad_res.error
    };
  }
  const Z_pad = Z_pad_res.value!;
  const l_i_b_str_res = i2osp(i32(lenInBytes), 2);
  if (l_i_b_str_res.error != null) {
    return {
      value: null,
      error: l_i_b_str_res.error
    };
  }
  const l_i_b_str = l_i_b_str_res.value!;
  const b = new Array<Uint8Array>(ell);
  const i2osp_res_2 = i2osp(0, 1);
  if (i2osp_res_2.error != null) {
    return {
      value: null,
      error: i2osp_res_2.error
    };
  }
  const b_0 = H(concatBytes([Z_pad, msg, l_i_b_str, i2osp_res_2.value!, DST_prime]));
  const i2osp_res_3 = i2osp(1, 1);
  if (i2osp_res_3.error != null) {
    return {
      value: null,
      error: i2osp_res_3.error
    };
  }
  b[0] = H(concatBytes([b_0, i2osp_res_3.value!, DST_prime]));
  for (let i = 1; i <= ell; i++) {
    const i2osp_loop_res = i2osp(i + 1, 1);
    if (i2osp_loop_res.error != null) {
      return {
        value: null,
        error: i2osp_loop_res.error
      };
    }
    const args = [strxor(b_0, b[i - 1]), i2osp_loop_res.value!, DST_prime];
    b[i] = H(concatBytes(args));
  }
  const pseudo_random_bytes = concatBytes(b);
  return {
    value: pseudo_random_bytes.slice(0, i32(lenInBytes)),
    error: null
  };
}

function hash_to_field(msg: Uint8Array, count: number): BI_2DArrayorError {
  ...
  if (htfOptions.expand) {
    const prb_res = expand_message_xmd(msg, DST, len_in_bytes, htfOptions.hash);
    if (prb_res.error != null) {
      return {
        value: null,
        error: prb_res.error
      };
    }
    pseudo_random_bytes = prb_res.value!;
  }
...
function map_to_curve_simple_swu_9mod16(t: Fp2): FP2ArrayorError {
  ...
  if (!success && !success2) {
    return {
      value: null,
      error: 'Hash to Curve - Optimized SWU failure'
    };
  }
  ...
  return {
    value: [div_res.value!, y],
    error: null
  };
}

function isogenyMap(COEFF: Array<Fp2_4>, x: Fp2, y: Fp2): FP2ArrayorError {
  ...
  x = x_res.value!;
  const y_div_res = yNum.div(yDen);
  if (y_div_res.error !== null) {
    return {
      value: null,
      error: y_div_res.error
    };
  }
  y = y.multiply(y_div_res.value!); // y * (yNum / yDev)
  return {
    value: [x, y],
    error: null
  };
}
const isogenyMapG2 = (x: Fp2, y: Fp2): FP2ArrayorError => isogenyMap(ISOGENY_COEFFICIENTS_G2, x, y);

class Fp12ArrayorError {
  value: Array<Fp12> | null;
  error: string | null;
}

function genInvertBatchFp12(nums: Fp12[]): Fp12ArrayorError {
  ...
  // Invert last element
  const inv_res = lastMultiplied.invert();
  if (inv_res.error !== null) {
    return {
      value: null,
      error: inv_res.error
    };
  }
  const inverted = inv_res.value!;
...
function calcPairingPrecomputes(x: Fp2, y: Fp2): FP2_2DArrayorError {
  // prettier-ignore
  ...
  for (let i = BLS_X_LEN - 2; i >= 0; i--) {
    ...
    const Rx_div_res = t0.subtract(t3).multiply(Rx).multiply(Ry).divBI(BI.fromI32(2));
    if (Rx_div_res.error !== null) {
      return {
        value: null,
        error: Rx_div_res.error
      };
    }
    Rx = Rx_div_res.value!; // ((T0 - T3) * Rx * Ry) / 2
    const Ry_div_res = t0.add(t3).divBI(BI.fromI32(2));
    if (Ry_div_res.error !== null) {
      return {
        value: null,
        error: Ry_div_res.error
      };
    }
    Ry = Ry_div_res.value!.square().subtract(t2.square().multiplyBI(BI.fromI32(3))); // ((T0 + T3) / 2)² - 3 * T2²
...
function normP1(point: Uint8Array): PointG1orError {
  return PointG1.fromHex(point);
}
function normP2(point: Uint8Array): PointG2orError {
  return PointG2.fromSignature(point);
}
function normP2Hash(point: Uint8Array): PointG2orError {
  return PointG2.hashToCurve(point);
}

function pairing(P: PointG1, Q: PointG2, withFinalExponent: boolean = true): Fp12orError {
  ...
  const res_P_validity = P.assertValidity();
  if (res_P_validity.error !== null) {
    return {
      value: null,
      error: res_P_validity.error
    };
  }
  const res_Q_validity = Q.assertValidity();
  if (res_Q_validity.error !== null) {
    return {
      value: null,
      error: res_Q_validity.error
    };
  }
  const looped_res = P.millerLoop(Q);
  if (looped_res.error !== null) {
    return {
      value: null,
      error: looped_res.error
    };
  }
  const looped = looped_res.value!;
  if (!withFinalExponent) {
    const fe_res = looped.finalExponentiate();
    if (fe_res.error !== null) {
      return {
        value: null,
        error: fe_res.error
      };
...
export function verify(signature: Uint8Array, message: Uint8Array, publicKey: Uint8Array): VerifyResult {
  const P_res = normP1(publicKey);
  if (P_res.error !== null) {
    return {
      value: false,
      error: P_res.error
  ...
  const P = P_res.value!;
  const Hm_res = normP2Hash(message);
  if (Hm_res.error !== null) {
    return {
      value: false,
      error: Hm_res.error
    };
  }
  const Hm = Hm_res.value!;
  const G = PointG1.BASE;
  const S_res = normP2(signature);
  if (S_res.error !== null) {
    return {
      value: false,
      error: S_res.error
  ...
  const S = S_res.value!;
  const ePHm_res = pairing(P.negate(), Hm, false);
  if (ePHm_res.error !== null) {
    return {
      value: false,
      error: ePHm_res.error
    };
  }
  const ePHm = ePHm_res.value!;
  const eGS_res = pairing(G, S, false);
  if (eGS_res.error !== null) {
    return {
      value: false,
      error: eGS_res.error
    };
  }
  const eGS = eGS_res.value!;
  const exp_res = eGS.multiply(ePHm).finalExponentiate();
  if (exp_res.error !== null) {
    return {
      value: false,
      error: exp_res.error
    };
  }
  const exp = exp_res.value!;
  return {
    value: exp.equals(Fp12.ONE),
    error: null
  };
}
