import {
    AllowedDonator,
  SuppliedEther, UpdatedEtherForCoverage, UpdatedSharesForCoverage, VoidedShares
...
export function handleSuppliedEther(event: SuppliedEther): void {
  ...
  const cseId = event.transaction.hash.toHexString() + "@" + event.transactionLogIndex.toString() + "@" + event.address.toHexString()
